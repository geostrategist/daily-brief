...
#
# Fired by Windows Task Scheduler Tue at 02:00. Leaves a draft in nuclear/_drafts/
...
$published = Join-Path $repo "nuclear\briefs\Brief_$Date.md"
...
try {
    ...
    if ($selfPublished) {
        ...
        $head = (& git -C $repo log -1 --format=%H -- "nuclear/briefs/Brief_$Date.md" 2>$null)
        if ($head) {
        ...
        & $py (Join-Path $repo "nuclear\_system\publish.py") $Date --dry-run --auto 2>&1 |
            ForEach-Object { Add-Content -Path $log -Value $_ -Encoding UTF8 }
        exit 0
    }
