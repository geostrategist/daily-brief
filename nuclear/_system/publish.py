...
The Tue 02:00 local run writes a draft and stops. Nothing reaches the site until this
script runs, so a bad brief is a file on disk rather than a published page.

    python nuclear/_system/publish.py              # today's draft
    python nuclear/_system/publish.py 20260820     # a specific date
    python nuclear/_system/publish.py --list       # what is waiting
    python nuclear/_system/publish.py --dry-run    # show the plan, touch nothing
...
ROOT = Path(__file__).resolve().parent.parent   # …/nuclear
REPO = ROOT.parent
DRAFTS = ROOT / "_drafts"
BRIEFS = ROOT / "briefs"
...
def check_draft(path):
    ...
    required = ["今日三分鐘摘要", "機組狀態速覽", "核能相關動態",
                "核子安全", "核子保安", "核子保防", "巡檢摘要"]
    ...
    for old, new in [("NS-R-1", "SSR-2/1"), ("NS-R-3", "SSR-1"),
                     ("GS-R-3", "GSR Part 2"), ("WS-R-5", "GSR Part 6")]:
        if old in text:
            warn.append(f"出現已作廢編號 {old}（現行為 {new}）")
    if "原能會" in text:
        warn.append("出現「原能會」——2023-09-27 已改組為核安會")
...
def main():
    ...
    if args.list:
        ...
        print(f"\n發布：python nuclear/_system/publish.py [YYYYMMDD]")
        return
    ...
    if target.exists() and not args.force:
        raise SystemExit(
            ...
            f"  git diff --no-index nuclear/briefs/{draft.name} nuclear/_drafts/{draft.name}"
        )
    ...
    print(f"  2. python nuclear/_system/rebuild-manifest.py")
    print(f"  3. git add nuclear/ && git commit -m 'nuclear: {iso}' && git push")
    ...
    run(["git", "commit", "-m", f"nuclear: {iso}"])
    print(f"已提交：nuclear: {iso}")
    ...
    print("已推送。")
    print(f"https://geostrategist.github.io/daily-brief/nuclear/ （GitHub Pages 約需一分鐘）")
    draft.unlink()
