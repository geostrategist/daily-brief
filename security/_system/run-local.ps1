...
#
# Manual test:
#   powershell -ExecutionPolicy Bypass -File security\_system\run-local.ps1
#   powershell -ExecutionPolicy Bypass -File security\_system\run-local.ps1 -Date 20260820
...
$drafts = Join-Path $repo "security\_drafts"
...
try {
    ...
    if (Test-Path $draft) {
        ...
        Say "review it, then: python security\_system\publish.py $Date"
    } else {
