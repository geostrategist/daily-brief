...
#
# Manual test:
#   powershell -ExecutionPolicy Bypass -File conflict\_system\run-local.ps1
#   powershell -ExecutionPolicy Bypass -File conflict\_system\run-local.ps1 -Date 20260820
...
$drafts = Join-Path $repo "conflict\_drafts"
...
$published = Join-Path $repo "conflict\briefs\Brief_$Date.md"
...
$promptFile   = Join-Path $repo "conflict\_system\DAILY_PROMPT.md"
$overrideFile = Join-Path $repo "conflict\_system\LOCAL_OVERRIDE.md"
...
try {
    ...
    if ($selfPublished) {
        ...
        $head = (& git -C $repo log -1 --format=%H -- "conflict/briefs/Brief_$Date.md" 2>$null)
        if ($head) {
        ...
        & $py (Join-Path $repo "conflict\_system\publish.py") $Date --dry-run --auto 2>&1 |
            ForEach-Object { Add-Content -Path $log -Value $_ -Encoding UTF8 }
        exit 0
    }
    ...
    Say "publishing"
    & $py (Join-Path $repo "conflict\_system\publish.py") $Date --auto 2>&1 |
        ForEach-Object { Add-Content -Path $log -Value $_ -Encoding UTF8 }
