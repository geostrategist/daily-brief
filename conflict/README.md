...
每週二、四巡檢並產出一份晨報。母站「實踐國企晨報」的子站，沿用同一套架構但有自己的
`_system/` 設定與 `briefs/`。
...
一個沒有後端的靜態網站。每週二、四由本機排程在 04:00 巡檢固定來源、比對前 21 份晨報、
...
_drafts/                週二、四 04:00 產出的草稿，未發布。已 gitignore
  logs/                 每次執行的完整輸出
_system/
  ...
  run-local.ps1         本機週二、四 04:00 排程執行的腳本，只產草稿
...
週二、四 04:00，Windows 工作排程器會跑 `_system/run-local.ps1`，巡檢來源、寫出草稿到
