...
#   powershell -ExecutionPolicy Bypass -File journals\_system\run-local.ps1 -Date 20260820
...
$drafts = Join-Path $repo "journals\_drafts"
...
$published = Join-Path $repo "journals\briefs\Brief_$Date.md"
...
# Pull the bibliographic data first. 49 Crossref queries belong in deterministic
# code, not in the agent: the agent's job is selection and judgement, not fetching.
# A failure here is fatal - a brief with no data is worse than no brief.
$py = (Get-Command python -ErrorAction SilentlyContinue).Source
if (-not $py) { Say "python not found on PATH"; Remove-Item $lock -Force -EA SilentlyContinue; exit 1 }
Say "fetching Crossref metadata"
& $py (Join-Path $repo "journals\_system\fetch.py") 2>&1 |
    ForEach-Object { Add-Content -Path $log -Value $_ -Encoding UTF8 }
$json = Join-Path $drafts ("crossref_" + $Date + ".json")
if (-not (Test-Path $json)) {
    Say "fetch.py produced no json, aborting"
    Remove-Item $lock -Force -ErrorAction SilentlyContinue
    exit 1
}
Say ("metadata ready: " + $json)

$claude = (Get-Command claude -ErrorAction SilentlyContinue).Source
if (-not $claude) { Say "claude CLI not found on PATH"; exit 1 }

$promptFile   = Join-Path $repo "journals\_system\DAILY_PROMPT.md"
$overrideFile = Join-Path $repo "journals\_system\LOCAL_OVERRIDE.md"
...
try {
    ...
    if ($selfPublished) {
        ...
        $head = (& git -C $repo log -1 --format=%H -- "journals/briefs/Brief_$Date.md" 2>$null)
        if ($head) {
        ...
        & $py (Join-Path $repo "journals\_system\publish.py") $Date --dry-run --auto 2>&1 |
            ForEach-Object { Add-Content -Path $log -Value $_ -Encoding UTF8 }
        exit 0
    }
