...
The Mon/Wed/Fri 04:00 local run writes a draft and stops. Nothing reaches the site until
...
def main():
    ...
    if warn:
        ...
        if stop:
            ...
            for w in stop:
                print(f"  ✖ {w}")
            print(f"  修好後再執行：python _system/publish.py {date}")
            raise SystemExit(1)
        print("  （以上為提醒，不阻擋發布）")
    else:
